...
import (
	...
	"github.com/SousaGLucas/apps/banking/ledger/app/service/api"
	"github.com/SousaGLucas/apps/banking/ledger/app/service/api/v1"
	"github.com/SousaGLucas/apps/banking/ledger/domain/usecases/account"
	"github.com/SousaGLucas/apps/banking/ledger/gateways/pg"
	"github.com/SousaGLucas/apps/banking/ledger/gateways/pg/migrations"
)
...
func Main(logger *zap.Logger, cfg config) error {
	...
	getBalanceUseCase := account.GetBalanceUseCase{
		DB: struct {
			pg.AccountsRepository
			pg.AccountEventsRepository
		}{
			pg.AccountsRepository{DB: pgPool},
			pg.AccountEventsRepository{DB: pgPool},
		},
	}
	cashInUseCase := account.CashInUseCase{
		DB: struct {
			pg.AccountsRepository
			pg.AccountEventsRepository
		}{
	...
	cashOutUseCase := account.CashOutUseCase{
		DB: struct {
			pg.AccountsRepository
			pg.AccountEventsRepository
		}{
	...
	apiV1 := v1.API{
		...
		CashInHandler:        v1.CashInHandler(cashInUseCase),
		CashOutHandler:       v1.CashOutHandler(cashOutUseCase),
	}
	apiV1.Routes(router)
