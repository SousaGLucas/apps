...
func WithdrawHandler(uc account.WithdrawUseCase) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = uc.Withdraw(ctx, account.WithdrawInput{
			AccountID: accountID,
			Amount:    body.Amount,
		})
		if err != nil {
			...
			case errors.Is(err, entities.ErrAccountNotFound):
				...
				return
			case errors.Is(err, entities.ErrNotEnoughFunds):
